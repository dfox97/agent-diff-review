...
/**
 * Agent-agnostic command runner. Mirrors the shape of `@earendil-works/pi-coding-agent`'s
 * `ExtensionAPI.exec`, but allows other agentic tools to plug in their own shell layer.
 */
...
async function runGit(exec: Exec, repoRoot: string, args: string[]): Promise<string> {
...
async function runGitAllowFailure(exec: Exec, repoRoot: string, args: string[]): Promise<string> {
	const result = await exec("git", args, { cwd: repoRoot });
	if (result.code !== 0) {
		return "";
	}
...
export async function getRepoRoot(exec: Exec, cwd: string): Promise<string> {
...
async function hasHead(exec: Exec, repoRoot: string): Promise<boolean> {
	const result = await exec("git", ["rev-parse", "--verify", "HEAD"], { cwd: repoRoot });
	return result.code === 0;
}

async function getMergeBase(exec: Exec, repoRoot: string, baseBranch: string): Promise<string | null> {
...
async function resolveBranch(exec: Exec, repoRoot: string, branch: string): Promise<string | null> {
...
export async function getReviewWindowData(
	...
	const commits = repositoryHasHead
		? parseCommitLog(
				mergeBase != null
					? await runGitAllowFailure(exec, repoRoot, ["log", `${mergeBase}..HEAD`, "--max-count=50", "--format=%H%x09%h%x09%s"])
					: await runGitAllowFailure(exec, repoRoot, ["log", "--max-count=50", "--format=%H%x09%h%x09%s"]),
			)
		: [];
	const commitChanges = new Map<string, ChangedPath[]>();
	for (const commit of commits) {
		const output = await runGitAllowFailure(exec, repoRoot, ["diff-tree", "--root", "--find-renames", "-M", "--name-status", "--no-commit-id", "-r", commit.sha]);
...
export async function loadReviewFileContents(
	...
	const originalContent = comparison.oldPath == null ? "" : await getRevisionContent(exec, repoRoot, originalRevision, comparison.oldPath);
